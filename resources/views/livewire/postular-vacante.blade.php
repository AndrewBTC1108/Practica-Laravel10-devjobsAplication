<div class="bg-gray-100 p-5 mt-10 flex flex-col justify-center items-center">
    <h3 class="text-center text-2xl font-bold my-4">Postularme a esta avacante</h3>

    @if (session()->has('mensaje') || session()->has('error'))
        @if (session()->has('error'))
            <div class="uppercase border border-red-600 bg-red-100 text-red-800 font-bold p-2 my-5 text-sm rounded-lg">
                {{session('error')}}
            </div>
        @else
